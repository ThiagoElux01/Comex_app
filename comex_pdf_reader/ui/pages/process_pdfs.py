...
def render():
                ...
                if acao == "duas":
                    ...
                    if df_final is not None and not df_final.empty:
                        ...
                        with col_xlsx:
                            xlsx_bytes = to_xlsx_bytes_externos_duas_abas(
                                df_normal=df_final,
                                sheet_normal="Percepciones",
                                sheet_spaced="Percepciones_Espacado",
                                blank_rows=3
                            )
                            st.download_button(
                                "Baixar XLSX (Percepciones)",
                                data=xlsx_bytes,
                    ...
                    if df_final is not None and not df_final.empty:
                        ...
                        with col_xlsx:
                            xlsx_bytes = to_xlsx_bytes_externos_duas_abas(
                                df_normal=df_final,
                                sheet_normal="Adicionales",
                                sheet_spaced="Adicionales_Espacado",
                                blank_rows=3
                            )
                            st.download_button(
                                "Baixar XLSX (Adicionales)",
                                data=xlsx_bytes,
    ...
    with tab3:
        ...
        if st.session_state.get("acao_selecionada") != "sharepoint":
            ...
            # remove qualquer herança da Tasa SUNAT
            st.session_state["tasa_df"] = None
    
            # garante uploader independente
        ...
        if uploaded_excel:
            try:
                ...
                if "all" not in xls.sheet_names:
                    st.error(
                        f"❌ A aba 'all' não foi encontrada. "
                ...
                with col_xlsx:
                    ...
                    with pd.ExcelWriter(buffer, engine="openpyxl") as writer:
                        ...
                        # aplica PRN widths apenas se fizer sentido
    ...
    with tab5:
        ...
        if flow == "externos":
            ...
            uploaded_xl = st.file_uploader(
                ...
                help="1ª aba -> Externos.prn/.xlsx \n 2ª aba -> aexternos.prn/.xlsx"
            )
        ...
        # -------------------- FLUXO DUAS --------------------
            ...
            uploaded_xl_d = st.file_uploader(
                ...
                key="prn_duas_upl",
                accept_multiple_files=False,
                help="1ª aba -> Duas.prn/.xlsx \n 2ª aba -> ADuas.prn/.xlsx"
            )
        ...
        # -------------------- FLUXO GASTOS ADICIONALES --------------------
            ...
            uploaded_xl_g = st.file_uploader(
                ...
                key="prn_gastos_upl",
                accept_multiple_files=False,
                help="1ª aba -> Adicionales.prn/.xlsx \n 2ª aba -> AAdicionales.prn/.xlsx"
            )
