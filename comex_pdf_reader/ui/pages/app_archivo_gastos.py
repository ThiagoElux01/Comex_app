...
def render():
    ...
    with col_b1:
        if st.button("Estado de Cuenta", use_container_width=True):
            _set_mode("estado"); st.session_state["aag_state"]["last_action"] = "estado"; st.rerun()
    with col_b2:
        if st.button("Plantilla Gastos", use_container_width=True):
            _set_mode("plantilla"); st.session_state["aag_state"]["last_action"] = "plantilla"; st.rerun()
    with col_b3:
        if st.button("Analise", use_container_width=True):
            _set_mode("asientos"); st.session_state["aag_state"]["last_action"] = "asientos"; st.rerun()
    with col_b4:
        if st.button("Cuenta", use_container_width=True):
            _set_mode("cuenta"); st.session_state["aag_state"]["last_action"] = "cuenta"; st.rerun()
    with col_b5:
        if st.button("Limpieza Plantilla Gastos", use_container_width=True):
            _set_mode("limpieza"); st.session_state["aag_state"]["last_action"] = "limpieza_pg"; st.rerun()
    ...
    if mode == "limpieza":
        ...
        try:
            # Base fixa: sempre a última versão limpa, se existir; senão, a original
            ...
            base_pg = df_pg_clean_current if (df_pg_clean_current is not None and not df_pg_clean_current.empty) else df_pg_orig

            # Cuenta usada: sempre o acumulado
            df_ct = st.session_state.get("aag_cuenta_df_accum", None)

            if base_pg is None or (hasattr(base_pg, 'empty') and base_pg is not None and base_pg.empty):
                ...
                # Executa limpeza
                df_pg_clean_new, stats = limpiar_plantilla_contra_cuenta(base_pg, df_ct, chave_col="Chave")
                ...
                with c1: st.metric("Linhas (Base)", f"{stats['rows_original']:,}".replace(",", "."))
                with c2: st.metric("Linhas (Limpo)", f"{stats['rows_clean']:,}".replace(",", "."))
                with c3: st.metric("Removidas", f"{stats['rows_removed']:,}".replace(",", "."))
                if stats["rows_removed"] == 0:
                    st.info("Nenhuma divergência de contagem encontrada. Nada foi removido.")

                # Botão desfazer permanece disponível
                if st.button("↩️ Desfazer última limpeza", use_container_width=False):
                ...
                # Analise pós-limpeza (sem filtros/alternâncias)
                st.divider(); st.subheader("🔍 Analise (Estado de Cuenta × Plantilla **limpa**)" )
                ...
                if df_ec is None or df_ec.empty:
                    ...
                    tol = st.number_input("Valor de Tolerância", min_value=0.00, value=0.01, step=0.01, key="limpieza_tol_fixed")
                    def _norm_conta(x) -> str:
                        s = re.sub(r"\D", "", str(x)); s = s.lstrip("0"); return s if s else ""
                    if "CTA" not in df_ec.columns or "Período" not in df_ec.columns:
                        st.error("Estado de Cuenta não contém as colunas esperadas: 'CTA' e 'Período'."); return
                    df_ec_proc = df_ec.copy(); df_ec_proc["CTA"] = df_ec_proc["CTA"].apply(_norm_conta)
                    ...
                    df_ec_agg = (
                        df_ec_proc.groupby("CTA", as_index=False)["Período"].sum()
                    ...
                    cuenta_col_clean = _find_col(df_pg_clean, "cuenta"); amount_col_clean = _find_col(df_pg_clean, "amount")
                    if cuenta_col_clean is None or amount_col_clean is None:
                        st.error("Plantilla (limpa) não contém as colunas esperadas: 'Cuenta' e 'Amount'."); st.stop()
                    df_pg_c = df_pg_clean.copy(); df_pg_c[amount_col_clean] = pd.to_numeric(df_pg_c[amount_col_clean], errors="coerce").fillna(0.0)
                    df_pg_c["__conta__"] = df_pg_c[cuenta_col_clean].apply(_norm_conta)
                    df_pg_c = df_pg_c[df_pg_c["__conta__"].astype(str).str.len() > 0]
                    df_pg_clean_agg = (
                        df_pg_c.groupby("__conta__", as_index=False)[amount_col_clean].sum()
                    ...
                    with col_d1:
                        st.download_button("Baixar CSV (Analise pós-limpeza)", df_show.to_csv(index=False).encode("utf-8"),
                                           "analise_pos_limpieza.csv", "text/csv", use_container_width=True)
                    with col_d2:
                        xlsx_bytes_cmp = to_xlsx_bytes_format(df_show, "AnalisePosLimpieza",
                                                              numeric_cols=["Saldo_Estado_Cuenta", "Saldo_Plantilla_Gastos", "Diferença"],
                                                              date_cols=[])
                        st.download_button("Baixar XLSX (Analise pós-limpeza)", xlsx_bytes_cmp,
                                           "analise_pos_limpieza.xlsx",
                                           "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
                                           use_container_width=True)

                # Prévia da Plantilla limpa
                if "aag_plantilla_df_clean" in st.session_state and isinstance(st.session_state["aag_plantilla_df_clean"], pd.DataFrame):
                    df_pg_prev = st.session_state["aag_plantilla_df_clean"]
                    col_csv, _ = st.columns(2)
                    with col_csv:
                        st.download_button("Baixar CSV (Plantilla Limpia)", df_pg_prev.to_csv(index=False).encode("utf-8"),
                                           "plantilla_gastos_limpia.csv", "text/csv", use_container_width=True)
        except Exception as e:
            st.error("Erro durante a limpeza da Plantilla de Gastos."); st.exception(e)
        return
    ...
    if mode == "estado":
        ...
        if clear_clicked:
            ...
            if "aag_estado_df" in st.session_state: del st.session_state["aag_estado_df"]
            st.rerun()
        if run_clicked and uploaded is not None:
            ...
            try:
                ...
                if df_base is None or df_base.empty:
                    st.warning("Nenhuma linha válida encontrada no arquivo."); pbar.progress(0, text="Aguardando..."); return
                st.session_state["aag_estado_df"] = df_base.copy()
                pbar.progress(70, text="Preparando visualização...")
                st.success("Arquivo processado com sucesso."); pbar.progress(100, text="Concluído.")
            except Exception as e:
                st.error("Erro ao processar o arquivo .txt."); st.exception(e)
        if "aag_estado_df" in st.session_state and isinstance(st.session_state["aag_estado_df"], pd.DataFrame):
            ...
            df = df_base.copy(); numeric_cols = ["Sal OB", "Saldo OB", "Período", "Saldo CB"]
            for c in numeric_cols: df[c] = pd.to_numeric(df[c], errors="coerce")
            totals = {c: float(np.nansum(df[c].values)) for c in numeric_cols}
            total_row = {col: "" for col in df.columns}; total_row["Descripción"] = "TOTAL"
            for c in numeric_cols: total_row[c] = totals[c]
            ...
            with col_csv:
                st.download_button("Baixar CSV (Estado de Cuenta)", df.to_csv(index=False).encode("utf-8"),
                                   "estado_de_cuenta.csv", "text/csv", use_container_width=True)
            with col_xlsx:
                xlsx_bytes = to_xlsx_bytes_format(df, sheet_name="EstadoCuenta", numeric_cols=numeric_cols, date_cols=[])
                st.download_button("Baixar XLSX (Estado de Cuenta)", xlsx_bytes, "estado_de_cuenta.xlsx",
    ...
    # ====== MODO: Plantilla ======
        ...
        uploaded_xl = st.file_uploader("Selecionar arquivo (.xlsx ou .xls)", type=["xlsx", "xls"], accept_multiple_files=False, key=upl_key_pg,
        ...
        if run_clicked and uploaded_xl is not None:
            ...
            try:
                name = getattr(uploaded_xl, "name", "").lower(); engine = "openpyxl" if name.endswith(".xlsx") else "xlrd"
                df_pg = pd.read_excel(uploaded_xl, sheet_name=0, engine=engine, dtype=str)
                amount_col = None
                for c in df_pg.columns:
                    if str(c).strip().lower() == "amount": amount_col = c; break
                if amount_col is None:
                    candidates = [c for c in df_pg.columns if "amount" in str(c).strip().lower()]
                    if candidates: amount_col = candidates[0]
                if amount_col is None:
                    st.error("Coluna 'Amount' não encontrada no arquivo."); return
                df_pg[amount_col] = pd.to_numeric(df_pg[amount_col], errors="coerce")
                def norm(s: str) -> str: return re.sub(r"[^a-z0-9]", "", str(s).strip().lower())
                date_targets = {"transactiondate": None, "duedate": None, "invoicedate": None}
                for c in df_pg.columns:
                    nc = norm(c)
                    if nc == "transactiondate": date_targets["transactiondate"] = c
                    elif nc in ("duedate", "due_date"): date_targets["duedate"] = c
                    elif nc in ("invoicedate", "invoice_date"): date_targets["invoicedate"] = c
                for key, col in date_targets.items():
                    if col and col in df_pg.columns: df_pg[col] = _to_datetime_from_mixed_excel_and_strings(df_pg[col])
                def _find_col_ci(df: pd.DataFrame, targets: list[str]):
                    ...
                    for t in targets:
                        ...
                        if key in cols_map: return cols_map[key]
                    return None
                ...
                def _ensure_series(x, n): return x if isinstance(x, pd.Series) else pd.Series([""] * n)
                df_pg["Chave"] = (
                ...
                pbar.progress(70, text="Preparando visualização...")
                st.success("Arquivo carregado com sucesso."); pbar.progress(100, text="Concluído.")
            except Exception as e:
                st.error("Erro ao processar o arquivo Excel."); st.exception(e)
        if "aag_plantilla_df_orig" in st.session_state and isinstance(st.session_state["aag_plantilla_df_orig"], pd.DataFrame):
            ...
            for c in df_pg.columns:
                if str(c).strip().lower() == "amount": amount_col_view = c; break
            if amount_col_view is None:
                candidates = [c for c in df_pg.columns if "amount" in str(c).strip().lower()]
                if candidates: amount_col_view = candidates[0]
            ...
            col_cfg = {}
            if amount_col_view: col_cfg[str(amount_col_view)] = st.column_config.NumberColumn(format="%.2f")
            for dc in found_date_cols_view:
                if pd.api.types.is_datetime64_any_dtype(df_pg[dc]): col_cfg[dc] = st.column_config.DateColumn(format="DD/MM/YYYY")
                else: col_cfg[dc] = st.column_config.TextColumn()
            col_csv, _ = st.columns(2)
            with col_csv:
                st.download_button("Baixar CSV (Plantilla Gastos)", df_pg.to_csv(index=False).encode("utf-8"),
                                   "plantilla_gastos.csv", "text/csv", use_container_width=True)
    ...
    # ====== MODO: Analise ======
        ...
        missing = []
        if df_ec is None or df_ec.empty: missing.append("Estado de Cuenta (.txt)")
        if df_pg is None or df_pg.empty: missing.append("Plantilla de Gastos (.xlsx/.xls)")
        if missing:
            st.warning("Para executar a análise, primeiro carregue: " + ", ".join(missing) + ". Use as abas **Estado de Cuenta** e **Plantilla Gastos**."); return
        ...
        try:
            ...
            df_ec_proc["Período"] = pd.to_numeric(df_ec_proc["Período"], errors="coerce").fillna(0.0)
            df_ec_proc = df_ec_proc[df_ec_proc["CTA"].astype(str).str.len() > 0]
            df_ec_agg = (df_ec_proc.groupby("CTA", as_index=False)["Período"].sum().rename(columns={"CTA": "Cuenta", "Período": "Saldo_Estado_Cuenta"}))
            pbar.progress(40, text="Consolidando Plantilla de Gastos...")
        except Exception as e:
            st.error("Erro ao consolidar Estado de Cuenta."); st.exception(e); return
        try:
            def _find_col(df, target):
                ...
                cand = [c for c in df.columns if target in str(c).strip().lower()]
                return cand[0] if cand else None
            cuenta_col = _find_col(df_pg, "cuenta"); amount_col = _find_col(df_pg, "amount")
            if cuenta_col is None or amount_col is None:
                st.error("Plantilla não contém as colunas esperadas: 'Cuenta' e 'Amount'."); return
            df_pg_proc = df_pg.copy(); df_pg_proc[amount_col] = pd.to_numeric(df_pg_proc[amount_col], errors="coerce").fillna(0.0)
            ...
            df_pg_agg = (df_pg_proc.groupby("__conta__", as_index=False)[amount_col].sum().rename(columns={"__conta__": "Cuenta", amount_col: "Saldo_Plantilla_Gastos"}))
            pbar.progress(70, text="Comparando saldos...")
        except Exception as e:
            st.error("Erro ao consolidar Plantilla de Gastos."); st.exception(e); return
        try:
            df_cmp = pd.merge(df_ec_agg, df_pg_agg, on="Cuenta", how="outer")
            for c in ["Saldo_Estado_Cuenta", "Saldo_Plantilla_Gastos"]: df_cmp[c] = pd.to_numeric(df_cmp[c], errors="coerce").fillna(0.0)
            df_cmp["Diferença"] = (df_cmp["Saldo_Plantilla_Gastos"] - df_cmp["Saldo_Estado_Cuenta"]).round(2)
            df_cmp["_div"] = df_cmp["Diferença"].abs() > float(tol)
            df_cmp["_cuenta_num"] = pd.to_numeric(df_cmp["Cuenta"], errors="coerce")
            df_cmp = df_cmp.sort_values(by="_cuenta_num", ascending=True).drop(columns=["_cuenta_num"]).reset_index(drop=True)
            ...
            c1, c2, c3 = st.columns(3)
            with c1: st.metric("Contas (Estado)", f"{df_ec_agg['Cuenta'].nunique():,}".replace(",", "."))
            with c2: st.metric("Soma Estado de Cuentas", f"{df_ec_agg['Saldo_Estado_Cuenta'].sum():,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            with c3: st.metric("Soma Plantilla de Gastos", f"{df_pg_agg['Saldo_Plantilla_Gastos'].sum():,.2f}".replace(",", "X").replace(".", ",").replace("X", "."))
            only_div = st.checkbox("Mostrar apenas contas com divergência", value=True)
            df_show = df_cmp[df_cmp["_div"]] if only_div else df_cmp
            df_show = df_show[["Cuenta", "Saldo_Estado_Cuenta", "Saldo_Plantilla_Gastos", "Diferença"]]
            st.dataframe(df_show, use_container_width=True, height=520,
            ...
            col_d1, col_d2 = st.columns(2)
            with col_d1:
                st.download_button("Baixar CSV (Analise)", df_show.to_csv(index=False).encode("utf-8"),
                                   "analise_contas.csv", "text/csv", use_container_width=True)
            with col_d2:
                xlsx_bytes = to_xlsx_bytes_format(df_show, "Analise",
                                                  numeric_cols=["Saldo_Estado_Cuenta", "Saldo_Plantilla_Gastos", "Diferença"], date_cols=[])
                st.download_button("Baixar XLSX (Analise)", xlsx_bytes, "analise_contas.xlsx",
                                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
            pbar.progress(100, text="Concluído.")
        except Exception as e:
            st.error("Erro durante a comparação."); st.exception(e)
    ...
    # ====== MODO: Cuenta ======
        ...
        with col_c:
            clear_clicked = st.button("Limpar acumulado", use_container_width=True)
        ...
        if run_clicked and uploaded is not None:
            ...
            try:
                ...
                st.error("Erro ao interpretar o arquivo GL0061."); st.exception(e); return
            if df.empty:
                st.error("Nenhuma linha reconhecida no arquivo GL0061."); return
            ...
            df_new = pd.concat([df_acc, df], ignore_index=True) if df_acc is not None else df.copy()
            ...
            st.success("Cuenta adicionada ao acumulado.")

        # Mostrar apenas um dataframe: Cuentas carregadas (acumulado)
        df_acc = st.session_state.get("aag_cuenta_df_accum", None)
        if df_acc is None or df_acc.empty:
            st.info("Nenhuma Cuenta acumulada ainda.")
        else:
            st.markdown("**Cuentas carregadas (acumulado)**")
            col_cfg = {
            ...
            st.dataframe(df_acc, use_container_width=True, height=520, column_config=col_cfg)
            col1, col2 = st.columns(2)
            with col1:
                st.download_button("Baixar CSV (acumulado)", df_acc.to_csv(index=False).encode("utf-8"),
                                   "cuentas_acumuladas.csv", "text/csv", use_container_width=True)
            with col2:
                xlsx_bytes = to_xlsx_bytes_format(df_acc, "CuentasAcumuladas",
                                                  numeric_cols=["Debe", "Haber", "Saldo Real", "Saldo"],
                                                  date_cols=[c for c in ["Fecha", "Fechado"] if c in df_acc.columns])
                st.download_button("Baixar XLSX (acumulado)", xlsx_bytes, "cuentas_acumuladas.xlsx",
                                   "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", use_container_width=True)
